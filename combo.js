async function fetchXML() {
    try {
        ...
        const comboCategory = xmlDoc.querySelector('category[product="combo"]');
        if (!comboCategory) throw new Error('Combo category not found');
        
        const comboList = document.getElementById('product-combo');
        comboList.innerHTML = ''; // Clear initial loading message
        
        comboCategory.querySelectorAll('list').forEach((product) => {
            ...
            comboList.appendChild(productContainer);
        });
    } catch (error) {
