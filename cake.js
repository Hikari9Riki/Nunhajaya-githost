async function fetchXML() {
    try {
        ...
        const cakeCategory = xmlDoc.querySelector('category[product="cake"]');
        if (!cakeCategory) throw new Error('Cake category not found');
        
        const cakeList = document.getElementById('product-cake');
        cakeList.innerHTML = ''; // Clear initial loading message
        
        cakeCategory.querySelectorAll('list').forEach((product) => {
            ...
            cakeList.appendChild(productContainer);
        });
    } catch (error) {
